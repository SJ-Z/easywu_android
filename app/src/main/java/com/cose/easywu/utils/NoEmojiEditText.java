        ...
        addTextChangedListener(new TextWatcher() {
            ...
            public void beforeTextChanged(CharSequence s, int start, int before, int count) {
                if (!resetText) {
                    ...
                    inputAfterText = s.toString(); // 这里用s.toString()而不直接用s是因为如果用s,
                ...
                if (!resetText) {
                    if (count >= 2) {// 表情符号的字符长度最小为2
                        ...
                        if (containsEmoji(input.toString())) {
                            resetText = true;
                            //暂不支持输入Emoji表情符号
                            ToastUtil.showMsgOnCenter(mContext, "暂不支持输入表情符号", Toast.LENGTH_SHORT);
                            setText(inputAfterText); // 是表情符号就将文本还原为输入表情符号之前的内容
