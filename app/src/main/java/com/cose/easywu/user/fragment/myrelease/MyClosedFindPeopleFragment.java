...
import com.cose.easywu.user.adapter.MyClosedFindPeopleAdapter;
...
public class MyClosedFindPeopleFragment extends BaseFragment {

    private SwipeRefreshLayout mSwipeRefresh;
    private RecyclerView mRv;
    private LinearLayout mLlNoGoods;
    private static ProgressDialog progressDialog;
    private MyClosedFindPeopleAdapter adapter;
    private String u_id;
    private SharedPreferences.Editor editor;

    private MyClosedFindPeopleFragment.MyHandler mHandler = new MyClosedFindPeopleFragment.MyHandler(this);
    private static int MSG_PICLEN = 0;
    private static int MSG_PIC = 1;

    @Override
    public View initView() {
        View view = View.inflate(mContext, R.layout.fragment_myclosed_findpeople, null);
        ...
        // 判断被用户关闭和被管理员关闭的失物招领总数是否为0
        if (LitePal.where("fg_state!=? and fg_state!=?", "0", "5").find(ReleaseFindPeople.class).size() == 0) {
    ...
    public void initData() {
        ...
        adapter = new MyClosedFindPeopleAdapter(mContext);
        adapter.setOnReleaseClick(new MyClosedFindPeopleAdapter.OnReleaseClick() {
            @Override
            public void click(ReleaseFindPeople goods) {
                handleRelease(goods);
            }
        });
        adapter.setOnDeleteClick(new MyClosedFindPeopleAdapter.OnDeleteClick() {
            @Override
            public void click(ReleaseFindPeople goods) {
                handleDelete(goods);
            }
    ...
    private void initListener() {
        mSwipeRefresh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                ...
                if (LitePal.findAll(ReleaseFindPeople.class).size() == 0) {
                    mLlNoGoods.setVisibility(View.VISIBLE);
                    mSwipeRefresh.setVisibility(View.GONE);
                } else {
    ...
    private void handleRelease(ReleaseFindPeople goods) {
        showProgressDialog();
        ...
        try {
            ...
            HttpUtil.sendPostRequest(Constant.REMOVE_FIND_GOODS_URL, json, new Callback() {
                ...
                public void onFailure(Call call, IOException e) {
                    ...
                    Log.e("MyClosedFindPeopleFrag", "删除失物招领失败:" + e.getMessage());
                }
                        ...
                        getActivity().runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (msg.getCode().equals("1")) {
                                    ...
                                    Log.e("MyClosedFindPeopleFrag", "删除失物招领成功");
                                } else {
                                    ToastUtil.showMsgOnCenter(mContext, "删除失败", Toast.LENGTH_SHORT);
                                    Log.e("MyClosedFindPeopleFrag", "删除失物招领失败");
                                }
                            }
                        });
                    }
