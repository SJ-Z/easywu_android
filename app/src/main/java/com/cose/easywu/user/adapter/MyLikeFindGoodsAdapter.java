...
public class MyLikeFindGoodsAdapter extends RecyclerView.Adapter<MyLikeFindGoodsAdapter.ViewHolder> {

    private Context mContext;
    private List<LikeFindGoods> likeGoodsList;
    private ArrayList<LikeFindGoods> removeGoodsList;
    private OnLikeClick onLikeClick;

    public MyLikeFindGoodsAdapter(Context context) {
        mContext = context;
        likeGoodsList = LitePal.findAll(LikeFindGoods.class);
        removeGoodsList = new ArrayList<>();
    }

    @NonNull
    @Override
    public MyLikeFindGoodsAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.item_mylikegoods, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final MyLikeFindGoodsAdapter.ViewHolder holder, final int position) {
        final LikeFindGoods goods = likeGoodsList.get(position);
    ...
    // 启动失物招领详情页面
    private void startGoodsInfoActivity(LikeFindGoods likeGoods) {
    ...
    public void setLikeGoodsList(List<LikeFindGoods> likeGoodsList) {
        this.likeGoodsList = likeGoodsList;
    }

    public List<LikeFindGoods> getRemoveGoodsList() {
