...
import com.cose.easywu.user.adapter.MyLikeFindGoodsAdapter;
...
public class MyLikeFindGoodsFragment extends BaseFragment {
    ...
    private MyLikeFindGoodsAdapter adapter;
    private String u_id;
    ...
    public View initView() {
        ...
        if (LitePal.findAll(LikeFindGoods.class).size() == 0) {
    ...
    public void initData() {
        ...
        adapter = new MyLikeFindGoodsAdapter(mContext);
        adapter.setOnLikeClick(new MyLikeFindGoodsAdapter.OnLikeClick() {
            @Override
    ...
    private void initListener() {
        mSwipeRefresh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                removeFromLitePal();
                adapter.setLikeGoodsList(LitePal.findAll(LikeFindGoods.class));
    ...
    private void removeFromLitePal() {
        List<LikeFindGoods> removeGoodsList = adapter.getRemoveGoodsList();
        if (removeGoodsList != null && removeGoodsList.size() > 0) {
            for (LikeFindGoods goods : removeGoodsList) {
                goods.delete();
            }
        }
    }
