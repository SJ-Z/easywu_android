...
import com.cose.easywu.user.adapter.MyReleaseFindGoodsAdapter;
...
public class MyReleaseFindGoodsFragment extends BaseFragment {

    private SwipeRefreshLayout mSwipeRefresh;
    private RecyclerView mRv;
    private LinearLayout mLlNoGoods;
    private static ProgressDialog progressDialog;
    private MyReleaseFindGoodsAdapter adapter;
    private String u_id;
    private SharedPreferences.Editor editor;

    private MyReleaseFindGoodsFragment.MyHandler mHandler = new MyReleaseFindGoodsFragment.MyHandler(this);
    private static int MSG_PICLEN = 0;
    private static int MSG_PIC = 1;

    @Override
    public View initView() {
        View view = View.inflate(mContext, R.layout.fragment_myrelease_findgoods, null);
        ...
        if (LitePal.where("fg_state=?", "0").find(ReleaseFindGoods.class).size() == 0) {
    ...
    public void initData() {
        ...
        adapter = new MyReleaseFindGoodsAdapter(mContext);
        adapter.setOnPolishClick(new MyReleaseFindGoodsAdapter.OnPolishClick() {
            @Override
            public void click(View view, TextView tvUpdateTime, ReleaseFindGoods goods) {
                setPolishToServer(tvUpdateTime, goods, (TextView) view);
            }
        });
        adapter.setOnDeleteClick(new MyReleaseFindGoodsAdapter.OnDeleteClick() {
            @Override
            public void click(ReleaseFindGoods goods) {
                handleDelete(goods);
            }
        });
        adapter.setOnEditClick(new MyReleaseFindGoodsAdapter.OnEditClick() {
            @Override
            public void click(ReleaseFindGoods goods) {
                handleEdit(goods);
            }
    ...
    private void handleEdit(ReleaseFindGoods goods) {
        showProgressDialog();
    ...
    private void deleteGoodsToServer(final ReleaseFindGoods goods) {
        ...
        try {
            ...
            HttpUtil.sendPostRequest(Constant.DELETE_FIND_GOODS_URL, jsonObject.toString(), new Callback() {
                @Override
                public void onFailure(Call call, IOException e) {
                    Log.e("MyReleaseFindGoodsFrag", "删除寻物启示失败:" + e.getMessage());
                    if (getActivity() != null) {
                        ...
                        getActivity().runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (msg.getCode().equals("1")) {
                                    ...
                                    Log.e("MyReleaseFindGoodsFrag", "删除寻物启示成功");
                                } else {
                                    ToastUtil.showMsgOnCenter(mContext, "删除失败", Toast.LENGTH_SHORT);
                                    Log.e("MyReleaseFindGoodsFrag", "删除寻物启示失败");
                                }
                            }
                        });
                    }
    ...
    private void setPolishToServer(final TextView tvUpdateTime, final ReleaseFindGoods goods, final TextView tvPolish) {
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("fg_id", goods.getFg_id());
            jsonObject.put("u_id", u_id);
            ...
            HttpUtil.sendPostRequest(Constant.POLISH_FIND_GOODS_URL, json, new Callback() {
                ...
                public void onFailure(Call call, IOException e) {
                    ...
                    Log.e("MyReleaseFindGoodsFrag", "擦亮失败:" + e.getMessage());
                }
                        ...
                        getActivity().runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (msg.getCode().equals("1")) {
                                    ...
                                    Log.e("MyReleaseFindGoodsFrag", "擦亮成功");
                                } else {
                                    ToastUtil.showMsgOnCenter(mContext, "擦亮失败", Toast.LENGTH_SHORT);
                                    Log.e("MyReleaseFindGoodsFrag", "擦亮失败");
                                }
                            }
                        });
                    }
